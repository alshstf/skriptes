...
#    SKRIPTES_VERSION=1.12.1              # тег релиза
...
- Импорт **идемпотентный**: повторный импорт того же файла — no-op (хэш-чек). Книга — это файл: пара «имя архива + `LIBID`», из какого бы INPX она ни пришла. Поэтому обновление приходит и под новым именем INPX: если раздача переименовала файл (lib.rus.ec в сентябре 2026: `librusec_local_fb2.inpx` → `librusec_mhl.inpx`), новый INPX продолжает те же книги — чтение, оценки и полки остаются на месте, добавляется только новое.
- Если в каталоге **одновременно** лежат два INPX одной библиотеки (lib.rus.ec кладёт `librusec_mhl.inpx` и `librusec_flib.inpx` с одними и теми же книгами), импортируется первый по алфавиту, а второй пропускается с записью `INPX skipped` в логе — иначе каждый выпуск импортировался бы дважды. Выберите нужный в `SKRIPTES_INPX_FILES`. Для lib.rus.ec это **`librusec_mhl.inpx`** — формат MyHomeLib, продолжение прежнего `librusec_local_fb2.inpx`; `librusec_flib.inpx` — те же книги в формате FLibrary, его совместимость со skriptes не проверялась.
- `INPX_HOST_PATH` можно указать прямо папкой торрент-раздачи (INPX ищется только в ней, без подкаталогов) вместе с `SKRIPTES_INPX_FILES`. Чтобы импорт не видел недокачанный файл, в qBittorrent включите «Добавлять расширение .!qB к незавершённым файлам» — тогда INPX появится под своим именем только целиком; без этого backend ждёт, пока файл перестанет меняться, и повторяет импорт, если тот упал на недописанном zip.
...
**Какой тег выбрать.** Кроме точных версий (`1.11.0`) публикуются движущиеся теги: `1.11` — последний патч ветки 1.11, `1` — последний релиз мажора 1, `latest` — последний стабильный релиз. Проект следует semver: всё, что при обновлении требует ручных действий (правка `.env`/compose, смена мажора PostgreSQL/Meilisearch, ручные шаги миграции), выходит только с новым мажором. Поэтому на теге `1` обновления можно ставить автоматически — например, `pull` + `up -d` по таймеру раз в неделю: ломающих изменений там не будет. Точный тег — если хотите обновляться только вручную.
...
⚠️ При публикации инстанса в интернет `/opds` наружу не выставляйте: `Caddyfile.public` пускает на него только из `SKRIPTES_LAN_CIDRS`, остальным отдаёт 404 (см. «Публичный доступ»); e-reader'ы — из домашней сети.
...
| `SKRIPTES_LAN_CIDRS` | — (закрыто для всех) | Только публичный деплой (`Caddyfile.public`): доверенные сети через пробел, откуда доступны админ-API и OPDS |
...
- **CSP** на всех страницах фронта: `script-src 'self'` без inline-скриптов. Ридер рендерит страницы книги во фреймах на том же origin, и они наследуют эту политику — скрипт или `onerror=` внутри fb2/EPUB не выполнится с правами сайта

Пока **нет**: 2FA, OIDC (поэтому в публичном деплое админ-API открыто только из `SKRIPTES_LAN_CIDRS`). Базовый сценарий — домашний сервер в доверенной сети; для публикации наружу используйте hardening-overlay + `Caddyfile.public` на изолированном хосте.
