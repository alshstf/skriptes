...
- Разделы **Авторы** (по умолчанию — «Сначала известные»: интегральная известность по работам автора; фильтры: жанры, языки, года активности, рейтинги, экранизации) и **Жанры**; статистика по автору (топ-жанры, серии, гистограмма по годам написания)
- **Служебные авторы** («Коллектив авторов», «Народные сказки», газеты/журналы) распознаются автоматически и не замусоривают список авторов и топ «плодовитых»; правится переключателем на карточке автора (админ)
- **Сборники отдельно от произведений**: антологии, авторские сборники и тома собраний сочинений распознаются автоматически (по названию, серии и числу авторов) и выносятся в секцию «Сборники и антологии» внизу карточки автора — романы не тонут среди «Избранных произведений», а «ПСС в 90 томах» не выглядит авторским циклом; сборники и их серии не раздувают счётчики автора и не влияют на его статистику (годы активности, чипсы жанров/языков); в профиле есть и жёсткий режим — «Скрывать сборники» убирает их из выдачи целиком (opt-in)
- Фильтры по жанру / языку / **языку оригинала** / году написания; персонализированный re-ranking по истории просмотров
- **«Золотая полка» по умолчанию**: без поискового запроса каталог упорядочен интегральной «известностью» книги — переиздания/переводы в коллекции, рейтинг LIBRATE, голоса Google Books / Open Library, экранизация, внешние счётчики известности (Фантлаб / Open Library / Wikidata — opt-in воркер «Известность» в админке) + ваши просмотры, прочтения и оценки; при близкой релевантности поиска известная книга тоже выигрывает
...
- **Название инстанса**: своё имя в заголовке Главной и вкладке браузера (админ → Общее)
...
- **Язык оригинала и переводчик** — из fb2 (`<src-lang>`, translator) → Wikidata («язык произведения», только однозначные ответы) для переводов, где fb2 оригинала не дал; на карточке — строка «Перевод с французского — …». Оригинал определяется на уровне логической книги: если хоть одно издание знает язык оригинала, он распространяется на все издания работы (перевод без fb2-метаданных не считается «оригиналом» на языке своего издания)
- **Внешний рейтинг** — Google Books / Open Library, когда в INPX нет LIBRATE
- **Биографии и портреты авторов** — Wikipedia REST API (полный intro section) + Open Library как fallback; матч защищён от однофамильцев двумя слоями — совпадение имени (не только фамилии) и профессия кандидата по Wikidata (писатель, а не тёзка другой профессии)
- **«Жизнь и книги»** — таймлайн на карточке известного автора: слева вехи биографии (утраты, война, аресты, переезды, браки), справа его книги напротив года написания. Наводя курсор на книгу, видно, что происходило в жизни автора, пока она писалась. Данные — Wikidata (даты) и биографическая секция Википедии (вехи с цитатами); секция появляется, только если событий и датированных книг достаточно, чтобы таймлайн был содержательным
...
#    SKRIPTES_VERSION=1.11.0              # тег релиза
...
| **Wikipedia REST API** | Биография автора (полный intro section), портреты; вехи жизни из текста биографической секции — с цитатами (для био-таймлайна; лицензия текста CC BY-SA 4.0, атрибуция ссылкой на статью) | — | [en.wikipedia.org/api/rest_v1/](https://en.wikipedia.org/api/rest_v1/) |
| **Wikidata SPARQL** | Экранизации книг (P144 "based on") с идентификаторами Кинопоиска/IMDb и постерами; год публикации (P577); QID работ (группировка); число sitelinks — в скольких Википедиях есть статья о книге (сигнал известности); датированные события жизни автора — браки, дети, утраты, переезды, награды (для био-таймлайна известных авторов; данные CC0) | — | [query.wikidata.org](https://query.wikidata.org/) |
