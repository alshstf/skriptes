...
/**
 * Sheet — выезжающая сбоку/снизу панель поверх overlay. Построена на том
 * же Radix Dialog primitive, что и ui/dialog.tsx (umbrella-пакет
 * `radix-ui`), но контент заякорен к краю экрана и анимируется слайдом, а
 * не центрируется с зумом.
 *
 * Используется для мобильных фильтров (/books): на десктопе фильтры —
 * sidebar, на мобильном — drawer слева по кнопке. Доступность, фокус-трап
 * и закрытие по Esc/overlay приходят из Radix бесплатно.
 */
  ...
  return (
    <SheetPortal>
      ...
      <SheetPrimitive.Content
        ...
        className={cn(
          "fixed z-50 flex flex-col gap-4 bg-background shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=open]:duration-300 data-[state=closed]:duration-200",
          sideClasses[side],
          className,
        )}
        {...props}
