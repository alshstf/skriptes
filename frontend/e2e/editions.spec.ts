...
test('book detail: секция «Издания» показывает все издания работы', async ({ page }) => {
  ...
  // Атрибуты обоих изданий. Переводчик ОТКРЫТОГО издания виден дважды: тихой
  // строкой «титульного листа» под сигналами (сокращённо) и в строке издания
  // (полностью, exact).
  await expect(page.getByText('Перевод — Вебер В.')).toBeVisible();
  await expect(page.getByText('Вебер Виктор', { exact: true })).toBeVisible();
