...
test('book detail: секция «Издания» показывает все издания работы', async ({ page }) => {
  ...
  // Атрибуты обоих изданий. Переводчик ОТКРЫТОГО издания виден дважды: в
  // строке сигналов («(пер.: …)» рядом с языком) и в строке издания (exact).
  await expect(page.getByText('(пер.: Вебер Виктор)')).toBeVisible();
