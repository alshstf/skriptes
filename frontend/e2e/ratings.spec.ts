...
// Проверяем: «Оценок пока нет» → клик по звезде → PUT /api/works/{id}/rating →
// после refetch видна средняя; всё в одном «оптимистичном» цикле react-query.
...
test('rating: ставит оценку на карточке и показывает среднюю после refetch', async ({
  ...
  await expect(page.getByText('Оценок читателей пока нет')).toBeVisible();
  ...
  // После refetch карточки появляется средняя оценка.
