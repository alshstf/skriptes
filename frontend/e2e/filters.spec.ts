...
test('filters: tri-state — клик по checkbox категории выбирает все leaf жанры', async ({
  ...
  // Полная категория схлопывается в ОДИН чип (аудит P2 #7) — не чип на каждый
  // поджанр (на проде клик по категории давал 24 чипа в 6 рядов).
  const categoryChip = page.getByText(/Жанр: Фантастика \(вся категория\)/);
  await expect(categoryChip).toBeVisible();
  await expect(page.getByText(/Жанр: Боевая фантастика/)).not.toBeVisible();
  await expect(page.getByText(/Жанр: Попаданцы/)).not.toBeVisible();

  // Крестик категорийного чипа снимает ВСЕ её жанры разом.
  await categoryChip.locator('..').locator('button').click();
  await expect(page).not.toHaveURL(/sf_action/);
  await expect(page).not.toHaveURL(/popadanec/);
});
