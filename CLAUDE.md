...
трогают. Текущая версия — **0.8.0** (стабильный; модель «книга → издания»:
works, группировка, схлопывание поиска/каталога, секция «Издания»; 0.8.0 —
отдельный works-индекс Meili (точные фасеты) + маршрут `/works/{id}`, books-индекс
остался для OPDS; 0.7.1 — фикс ридера, work-level favorite/read, обложка из любого
издания); `:latest` опубликован, в `.env` можно пинить `SKRIPTES_VERSION=latest`.
