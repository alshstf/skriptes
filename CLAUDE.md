...
| Фоновые операции (админка) — обработка коллекции (fb2) + внешние источники | Локальная джоба: `metadata/prewarm.go` (под-тумблеры covers/annotations/years + интенсивность + авто-ресинк) · Внешняя: `metadata/year_backfill.go` + `metadata/openlibrary_provider.go`/`wikidata_year.go` (`FetchYear`) · API: `api/admin_settings.go` (cover-cache=обработка) + `api/admin_year_enrichment.go` · Фронт: `frontend/src/pages/AdminBackgroundPage.tsx` |
