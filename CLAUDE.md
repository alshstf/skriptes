...
- `books.written_year` — год написания / первого издания произведения.
  Источники по приоритету: fb2 `<title-info><date>` (локально, в прогреве —
  `metadata.EnsureYearLocal` ← `fb2_provider.go::FetchYears`) → OpenLibrary
  `first_publish_year` → Wikidata `P577` (внешний rate-limited backfill-воркер
  `metadata.YearBackfillController`, opt-in из админки «Год издания»; per-source
  учёт попыток в таблице `book_year_lookups`, чтобы не долбить источник). По нему
  строится гистограмма «Книги по годам написания» (`catalog/service.go`); сюда же
  ляжет будущая bio-корреляция. `written_year_source` хранит происхождение.
...
Текущая верхняя — `0013_book_year_lookups` (per-source учёт внешнего
дозаполнения года). Backend хранит applied version в `schema_migrations`
(golang-migrate), править уже-применённые .sql имеет смысл только до push'а.
...
трогают. Текущая версия — **0.5.0** (первый стабильный); `:latest` опубликован,
...
| Год: внешний backfill (OL/Wikidata) + админка | `metadata/year_backfill.go` (worker+controller+rate-gate) · `metadata/openlibrary_provider.go` + `metadata/wikidata_year.go` (`FetchYear`) · `api/admin_year_enrichment.go` · `frontend/src/pages/AdminYearEnrichmentPage.tsx` |
...
| Видимость контента (скрыть жанры/языки) | `backend/internal/settings/content.go` + `backend/internal/api/content.go` + `frontend/src/components/ContentVisibility.tsx` (Admin/Profile content pages) |
