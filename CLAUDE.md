...
- **Внешний (НЕ читательский)** — единый «Внешний рейтинг» (иконка `Globe`), объединяет
  ...
  - web — `books.external_rating`/`_source`/`_count` (Google Books/OpenLibrary), миграция
    **0026**, заполняется фоновым воркером обогащения (PR B — пока поля пустые).
...
`BookHeart` — оценка читателей; колокольчик `Bell` — подписка на авторов/серии. Осталось
(роадмап, 2026-06-21): **PR B** — фоновый воркер онлайн-обогащения внешнего рейтинга
(Google Books/OpenLibrary, зеркало `cover_backfill.go`, per-source конфиг + охват в админке
«Фоновые операции»).
