...
трогают. Текущая версия — **0.9.0** (стабильный; модель «книга → издания»:
works, группировка, схлопывание поиска/каталога, секция «Издания»; 0.9.0 —
Tier-1.5 (том серии ⇒ работа) + ручной merge/split и подсказки на карточках
(админ); 0.8.0 — отдельный works-индекс Meili (точные фасеты) + маршрут
`/works/{id}`); `:latest` опубликован, в `.env` можно пинить `SKRIPTES_VERSION=latest`.
