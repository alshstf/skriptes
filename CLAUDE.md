...
трогают. Текущая версия — **1.4.1** — хотфикс: `/api/genres` отдавал 500 на большой
...
хендлера. Переписан на ОДИН `GROUP BY`-проход по `book_genres` (CTE) → миллисекунды. До неё
