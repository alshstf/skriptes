...
Текущая верхняя — `0025_rating_prompts` (`reads.acquired_at` + `book_rating_prompts`
для отложенных запросов оценки, см. граблю №17); до неё `0024_book_ratings`
...
трогают. Текущая версия — **1.2.1** (полировка рейтингов, граблю №17: иконки
`Landmark` (рейтинг библиотеки) / `BookHeart` (оценка читателей) вместо звезды —
звезда только избранному; те же иконки в фильтрах `/authors`; заголовок Главной →
«Skriptes»). До неё **1.2.0** — **пользовательские оценки книг** — см. граблю
...
- **Библиотечный (донорский)** — `books.rating` = LIBRATE из INPX (рейтинг librusec/
  flibusta). Только из импорта. На карточке поле «Рейтинг библиотеки», в Авторах
  фильтр `MinRating`/sort=`rating`/бейдж (`catalog/authors_list.go`).
- **Читательский (этого инстанса)** — пользовательские оценки 1–5, work-level:
  - `book_ratings(user_id, work_id, rating)` (миграция **0024**); сервис —
    ...
    блок «Оценки читателей» (`BookDetailPage::RatingsBlock` + `RatingControl` —
    числовая шкала 1–5, НЕ звезда); поля карточки `user_rating`/`rating_avg`/
    `rating_count` (`api/books.go::bookResponse`). API `PUT/DELETE /api/works/{id}/rating`.
...
⚠️ Иконки/нейминг ПУТАЮТСЯ (звезда библ.рейтинга ≈ избранное; «рейтинг»/«библиотека»
≈ «Библиотека Skriptes») — запланирован отдельный наведём-порядок-PR + онлайн-
обогащение внешнего рейтинга (роадмап, 2026-06-21). Звезду по-хорошему оставить
только избранному.
...
| Раздел «Авторы» (список + фильтры) | `frontend/src/pages/AuthorsPage.tsx` + `frontend/src/lib/authors.ts` · бэк: `catalog/authors_list.go::ListAuthorsFiltered` + `api/authors.go` (только авторы с ≥1 видимой книгой; фильтр языка матчит lang∪src_lang) |
| Раздел «Жанры» + личные полки | `frontend/src/pages/GenresPage.tsx` + `frontend/src/lib/collections.ts` + `components/AddToShelfDialog.tsx` · бэк: `internal/collections/service.go` + `api/collections.go`; избранное жанров — `history` + `catalog.ListGenres(userID)` |
