...
Текущая верхняя — `0039_book_identity_archive_libid` (книга = (архив, lib_id), схлопывание дублей из двух INPX одной библиотеки, см. карту «INPX → upsert»); до неё `0038_author_renown`; до неё `0037_adaptation_tmdb` (`book_adaptations.tmdb_movie_id`/
...
«Экранизации» — см. граблю №11/постеры); до неё `0036_service_authors`
(`authors.is_service`/`is_service_source` — служебные авторы-агрегаты вне
списка /authors); до неё `0035_book_src_lang_lookups` (учёт попыток
Wikidata-backfill src_lang); до неё `0034_work_kind` (`works.kind`/`kind_source` — тип работы:
...
**⚠️ Канонический шаблон вотчера — выстрадан (2026-07-19, «#225 давно ready,
а вотчер молчал»). Два бага, которые НЕЛЬЗЯ повторять:**
1. **Молчание на успехе.** Вотчер, эмитящий события только на MERGED/CLOSED/FAIL,
   не скажет «CI зелёный, ready to merge» — а это ГЛАВНОЕ actionable-событие
   (пользователь ждёт его для ревью/мержа). Событие — на КАЖДОЕ терминальное
   состояние: all-pass (один раз!), fail, merged, closed.
2. **Битый парсинг `gh pr checks | awk '{print $2}'`.** Имена чеков содержат
   пробелы («backend (1.25)») → $2 при дефолтном FS — кусок имени, не статус.
   ТОЛЬКО `--json`: `gh pr view N --json state,statusCheckRollup`.

```bash
# Monitor (persistent): события — READY (все чеки зелёные, один раз) / CI FAIL / MERGED / CLOSED
ready_said=""
while true; do
  j=$(gh pr view $PR --json state,statusCheckRollup 2>/dev/null) || { sleep 60; continue; }
  st=$(echo "$j" | python3 -c "import json,sys; print(json.load(sys.stdin)['state'])")
  [ "$st" = "MERGED" ] && { echo "[pr$PR] MERGED"; break; }
  [ "$st" = "CLOSED" ] && { echo "[pr$PR] CLOSED без merge"; break; }
  concl=$(echo "$j" | python3 -c "import json,sys; print(' '.join((c.get('conclusion') or c.get('status') or '?') for c in json.load(sys.stdin)['statusCheckRollup']))")
  case "$concl" in
    *FAILURE*|*ERROR*) echo "[pr$PR] CI FAIL: $concl"; break ;;
    *) if [ -z "$ready_said" ] && [ -n "$concl" ] && ! echo "$concl" | grep -qE "IN_PROGRESS|QUEUED|PENDING|\?"; then
         echo "[pr$PR] CI ЗЕЛЁНЫЙ — ready to merge"; ready_said=1
       fi ;;
  esac
  sleep 60
done
```
Смежные хронические грабли вотчеров: переменную НЕ называть `status` (zsh
read-only → молчаливый exit 1); `docker logs -f` умирает вместе с контейнером
при деплое/рестарте — прод-вотчер логов оборачивать в reconnect-цикл или
переармировать после каждого деплоя.
...
**Semver-контракт: ломающее изменение = новый мажор (правило владельца, 2026-09-25).**
Публичный (DMZ) инстанс сидит на движущемся теге `1` и **сам** обновляется раз в неделю
(`docker compose pull && up -d` по таймеру, без человека). Поэтому всё, что при обновлении требует
ручного действия или ломает существующий деплой, выпускается ТОЛЬКО следующим мажором (`2.0.0`),
не минором/патчем. **Ломающее:** удалён/переименован env или его семантика/дефолт изменились так,
что старый `.env` работает иначе; нужна правка compose/Caddyfile/`.env`/томов; смена мажора
PostgreSQL/Meilisearch (дамп/миграция данных); миграция БД, которая не применяется сама на старте
или требует ручных шагов; удалён/несовместимо изменён внешний интерфейс (URL OPDS, `/api/*` для
внешних клиентов); выросли минимальные требования к хосту. **НЕ ломающее:** новые фичи, новые env
с безопасным дефолтом, обычные авто-миграции и ресинки индексов на старте (даже тяжёлые — но
отметить в release notes). Сомневаешься — считай ломающим и спроси владельца. В PR с ломающим
изменением — пометка `BREAKING` и что сделать при обновлении. `infra/.env.public.example` держит
`SKRIPTES_VERSION=1` — при релизе его НЕ бампать.
...
трогают. Текущая версия — **1.12.1** — хотфикс импорта (#243): кэш id авторов/серий/жанров/
архивов в импортёре двухуровневый (id из откаченной транзакции записи больше не отравляет кэш →
каскад FK-ошибок) + дубль автора в записи схлопывается; на librusec-INPX это вернуло 22 книги,
которых не было ни в одном инстансе. ⚠️ После деплоя книги появятся при следующем импорте
изменённого INPX; сразу — сбросить `collections.last_inpx_hash` и перезапустить backend (полный
импорт ~70 мин). До неё **1.12.0** — **безопасность под прямую публикацию из DMZ**
...
`src_lang` её изданий, фолбэк на языки изданий ТОЛЬКО когда src нет ни у кого — испанский
перевод-сирота без fb2 `<src-lang>` больше не объявляет Остин «оригинал: испанский»; та же
семантика в `ListSrcLanguages` (один GROUP BY по `COALESCE(work_id,-id)`, счётчик = работы)
и в authors `src_langs` (work-level EXISTS с sibling-пробами). (#224) **бейдж «N изданий»
/books = ВИДИМЫЕ живые издания** (живой count, зеркало visibleEditions; не колонка
`works.edition_count`) — консистентность с карточкой при скрытых языках. (#225) **постеры
экранизаций из TMDB** (источник Plex; id бесплатно из SPARQL P4947/P4983 — без поиска;
`metadata/tmdb.go`, ключ env `SKRIPTES_TMDB_API_KEY` — v3 И v4-токен автодетектом,
собственный кламп tmdbRPM=600 в провайдере, атрибуция в README) + миграция **0037**
(персист tmdb-id + `poster_checked_at`) + **авто-перепроверка постер-дыр** по TTL 7 дней
(`RecheckPosterHoles`, фаза воркера «Экранизации» — постеры новых фильмов приходят сами)
+ три фикса необратимой потери постеров (транзиент не ставит маркер; «Очистить постеры»
сбрасывает маркер; кнопка «Дозаполнить постеры» = разовый бутстрап legacy-строк).
⚠️ После деплоя: ресинк v8 на старте (штатно); задать TMDB-ключ и нажать «Дозаполнить
постеры» (жмёт пользователь). До неё **1.9.4** — **закрытие хвоста аудита: служебные авторы + приоритизация
обогащения + P3-мелочи**, 3 PR (#219–#221), ночная автономная сессия 2026-07-19. (#220)
**служебные авторы**: агрегаты-псевдоавторы («Коллектив авторов» 805, «Народные сказки» 741,
«Газета Завтра» 737, «неизвестный автор» 504) — миграция **0036** `authors.is_service` +
`is_service_source` ('heuristic'|'manual'); эвристика `metadata/service_authors.go::
ClassifyServiceAuthors` (advisory-lock от гонки runOnce/after-import — зеркало work_kind;
консервативные якорные фразы + префиксы «газета/журнал/альманах/редакция/издательство » С
продолжением), runOnce-гейт `service_authors_classified_v1` + after-import; скрыты ТОЛЬКО из
...
(#221) **приоритизация обогащения «ядро сначала»**: двухфазный drain year/src_lang-воркеров —
сначала кандидаты ядра (`bookCoreCond`, `metadata/enrichment_priority.go`: работа с ≥2 изданиями ∪
экранизация ∪ LIBRATE — book-level зеркало candidateCond «Известности»), потом хвост; живой замер
прода: голова books = litnet-самиздат, первые 716 кандидатов src_lang — все not_found; охват не
меняется, только порядок. (#219) **P3-мелочи**: «К карточке» из ридера → `/works/{work_id}`
(кэш-first + фолбэк); **work-wide «снять прочитано»** — `UnmarkRead` снимает `completed_at` со
ВСЕХ изданий работы (корень «снять не срабатывало»: карточка показывает work-level статус, а
DELETE снимал только с представительного издания); выпилен мёртвый `history.RecordRead`. До неё
**1.9.3** — **язык оригинала как полноценная ось + UX-мелочи аудита**,
4 PR (#214–#217). (#216) **фильтр «Язык оригинала» = ЭФФЕКТИВНЫЙ оригинал**: works-индекс несёт
производное `orig_lang[]` = `src_lang ?? lang` (натив сам себе оригинал; схема works-индекса
v6→**v7**, полный ресинк на старте) — «оригинал: французский» ловит и переводы с французского, и
нативно-французские книги; опции фильтра (`ListSrcLanguages` + фасет `orig_lang`) — тоже
эффективные (появился «русский» для всей нативной русской литературы); URL-параметр значения
остался `?src_lang=` (back-compat); сырой `src_lang` не тронут — «Перевод с …» только у
переводов. (#217) **внешний backfill src_lang из Wikidata** (P407 «язык произведения»,
`metadata/src_lang_backfill.go` + `wikidata_srclang.go`, миграция **0035** `book_src_lang_lookups`):
источник ТОЛЬКО Wikidata — у OL поля оригинала нет (languages = union изданий, у «Войны и мира»
16 языков); два precision-гейта — «ровно один ISO-код у P407» (мультиязычную «Войну и мир»
ru+fr отвергаем) и «код ≠ язык издания» (нативы src_lang не получают); opt-in секция «Язык
оригинала» в админке «Фоновые операции», таргетный ресинк works-индекса. (#214) **имя
инстанса** — app-setting `branding` + таб «Общее» (`/admin/general`), `/api/version` отдаёт
`instance_name`, заголовок Главной и `<title>` вкладки из него. (#215) **возврат из «Настроить
Kindle»** на карточку (`returnTo` + navigate(replace), анти-open-redirect валидация) + **плавная
обложка** (fade-in в BookCover вместо резкого выскакивания; кэш coverOutcome без повторного
фейда). До неё **1.9.2** — **точность обогащения авторов: слой 2 (профессия P106)**,
2 PR (#211–#212). Имя-гейт (`authorNameMatches`, слой 1) пропускает однофамильца с ТЕМ ЖЕ ФИО,
но другой профессией (писатель vs. тёзка-политик/спортсмен). Слой 2 добивает: после имя-гейта
резолв автора спрашивает Wikidata о профессии кандидата (P106) и отсекает явных не-писателей.
Реализация гейта — `WikidataAdaptationsProvider.OccupationVerdict` (`metadata/wikidata_occupation.go`):
один SPARQL считает ВСЕ занятости `P106` и писательские (класс через `P279*` доходит до writer
Q36180 / author Q482980 — подклассы novelist/poet/playwright ловятся обходом сами). Вердикт:
`Writer`→принять, `NonWriter` (профессии есть, писательской нет)→**отвергнуть**, `Unknown` (нет
P106 / нет QID / ошибка сети)→**не отвергать** (precision-preserving, транзиент не теряет автора).
PR1 (#211) — гейт на **Wikipedia**-пути (`resolveTitle`; QID через доп. запрос `pageprops.wikibase_item`).
PR2 (#212) — гейт на **OpenLibrary**-пути (`authorSearch`; QID БЕСПЛАТНО из `remote_ids.wikidata`
детальной записи — доп. запроса не нужно). Оба провайдера получают ОДИН гейт в `main.go`
(`WithOccupationGate`). ⚠️ OL-гейт важен из-за цепочки `[wikipedia, openlibrary]`: если wiki-гейт
отверг однофамильца, enricher идёт к OL — без OL-гейта wiki-отказ «протёк» бы в OL-fallback. Гейт
опционален (nil = выкл). Грабля №13. ⚠️ Уже сохранённые ДО гейта неверные фото/био он не чистит —
поможет «Очистить фото» в админке (перекачает через гейт). До неё
**1.9.1** — **добивка внешнего обогащения + фикс гонки классификации**,
3 PR (#207–#209). (#207) **дневной кап вызовов Google Books** — free-квота GB ≈1000/сутки;
воркер рейтинга держит настраиваемый предел (`ExternalRatingConfig.GoogleBooksDailyCap`, дефолт
1000, поле «Google Books, вызовов/сутки» в админке «Внешний рейтинг»), считает вызовы за UTC-сутки
по `book_external_rating_lookups` (пере-сеивается при рестарте), сверх — GB пропускается без записи,
OL идёт как обычно → без 429-шторма. (#208) **обязательный `country=US`** во всех GB-вызовах
(`GoogleBooksProvider.WithCountry`/`addParams`, env `SKRIPTES_GOOGLE_BOOKS_COUNTRY`, дефолт US):
без него облачный IP ловит «Cannot determine user location…» (пусто) — разбор
groups.google.com/g/google-appengine/c/C-IoRG7Z7VI; **убран `langRestrict`** (резал валидные
переводные издания); `projection` не трогаем (дефолт full — в lite нет рейтинга). (#209)
**advisory-lock против deadlock классификации** сборников на первом деплое: `runOnce`-классификация
(горутина старта) и after-import классификация (в `runStartupImport`) стартовали одновременно, обе
делали три полнотабличных `UPDATE works` в разном порядке блокировки строк → deadlock, один вызов
падал (данные уцелели — второй успевал расставить `kind`). `ClassifyWorkKinds` теперь берёт
session-level `pg_advisory_lock` (ключ `workKindClassifyLockID`) на отдельном соединении и гонит все
`UPDATE` по нему — второй вызов ЖДЁТ первый, проходит по 0 строк (идемпотентно). ⚠️ После деплоя:
...
**Постеры экранизаций — источник TMDB (приоритет) → Commons P18 (фолбэк).**
Wikidata-P18 у фильмов почти пуст (постеры копирайтные; на реальном проде
покрытие было ~16%, у «Разум и чувства»/«Ёжика» — ноль). TMDB-id приходит
БЕСПЛАТНО из того же SPARQL-ответа адаптаций (P4947 фильм / P4983 сериал) —
поиска по названию нет, ложных матчей нет; `metadata/tmdb.go::TMDBPosterProvider`
(GET /3/movie|tv/{id} → poster_path → image.tmdb.org/t/p/w342, ключ env
...
необратимой потери постеров (все три были багами): (1) `saveAdaptations` НЕ
ставит `adaptations_fetched_at`, если постер-источники были, а ни одно
скачивание не удалось (транзиент: 429/сеть/ErrCacheFull) — строки пишутся,
книга остаётся pending, следующий заход дозальёт через ON CONFLICT COALESCE;
(2) «Очистить постеры» (`ClearPosterCache`) теперь сбрасывает и маркер у книг
с экранизациями — семантика «очистить и перекачать», раньше была необратимая
потеря (воркер берёт только NULL-маркер); (3) админ-кнопка **«Дозаполнить
постеры»** (`RefillMissingPosters`, POST
`/admin/adaptation-enrichment/refill-posters`) — сброс маркера ТОЛЬКО у книг с
постер-дырами → воркер «Экранизации»/lazy перепройдёт и дозальёт (жать после
включения TMDB-ключа на существующей коллекции — разовый бутстрап, заодно
персистит TMDB-id старым строкам). **Авто-перепроверка постер-дыр** (миграция
0037): TMDB-id (`tmdb_movie_id`/`tmdb_tv_id`) персистятся при записи адаптации
→ фаза `RecheckPosterHoles` воркера «Экранизации» (каждый 30-мин цикл, батч
500) перепроверяет дыры чистым TMDB-вызовом БЕЗ SPARQL с поштучным TTL
`poster_checked_at` (7 дней, `posterRecheckTTL`) — постер совсем нового фильма
(«честный ErrNotFound» в момент первого фетча) появится сам, без кнопки.
Found → дыра закрыта; всё ещё нет → следующая проверка через TTL; транзиент →
checked_at не трогается (ретрай в следующем цикле). Без TMDB-ключа фаза no-op.
⚠️ Остаточная staleness: НОВЫЕ экранизации у уже-фетченных книг single-shot
маркер не увидит (TTL-перекраул самих адаптаций — follow-up, SPARQL-бюджет).
...
только кандидатов, которых ПОРА спросить хотя бы у одного включённого источника —
`dueCond` + `dueArgs` (`metadata/lookup_due.go`, те же правила, что `lookupTTL.isDue` в Go;
контракт SQL≡Go держит `TestDueCond_MatchesIsDue`). Раньше срок проверялся только в Go
(`processOne → isDue → continue`), и проход раз в 30 минут перечитывал ВСЕХ кандидатов
охвата батчами с `array_agg` авторов — DMZ (552 тыс. изданий) держал Postgres на ~100% CPU
при нуле запросов наружу (#244). Новый воркер с lookups-таблицей — только так. Источник,
который сейчас спрашивать нельзя (GB за дневной квотой), в список `dueArgs` не передаётся.
Лог `pass complete` пишется только при реальных запросах: `candidates` (выбрано) и
`lookups` (запросов к источникам).
...
| INPX → upsert в PG + Meili | `backend/internal/importer/` — импорт при старте и без рестарта: `cmd/skriptes/main.go::runImportLoop` + `importer.InpxWatch` (опрос размера/mtime раз в `SKRIPTES_INPX_WATCH_INTERVAL`, дефолт 10m; КАЖДЫЙ файл отдаётся, когда сам не менялся между двумя проверками — недописанный zip не трогаем, соседи не задерживают; обработанным файл становится только через `MarkDone` после удачного `Run` или пропуска-дубля — упавший импорт повторяется на каждой проверке; один цикл на процесс → импорты не пересекаются); `SKRIPTES_INPX_FILES` — явный список файлов. **Книга = файл: UNIQUE (archive_id, lib_id), архив уникален по имени** (миграция 0039; `collection_id` у books/archives — «какой INPX описал последним», коллекция = имя INPX-файла только для хэша/версии). Переименованный раздачей INPX продолжает те же строки (id книг и данные пользователей сохраняются). Если ≥½ первых 1000 записей INPX числятся за ДРУГИМ INPX и тот тоже в каталоге (и в `SKRIPTES_INPX_FILES`, если список задан) — пропуск: `importer.OverlapError`, WARN `INPX skipped` (#250: lib.rus.ec кладёт `librusec_mhl` = формат MyHomeLib = продолжение local_fb2 и `librusec_flib` = те же книги в формате FLibrary, со skriptes не проверялся). Миграция 0039 схлопнула существовавшие дубли (перенос чтения/просмотров/полок, оценок опустевших работ) и оставила в `app_settings.book_identity_dedup_v1` список для чистки поиска — её на старте делает `importer.PurgeDedupedDocs`. Каждая запись в своей транзакции; кэши id авторов/серий/жанров/архивов (`cache.go`) ДВУХУРОВНЕВЫЕ: новый id лежит в `staged` и попадает в общий кэш только после Commit записи (иначе id из отката = FK-ошибки у всех следующих книг того же автора; прод 2026-09-26 — 22 книги); дубль автора в записи схлопывается |
...
| Язык оригинала (src_lang) — фасет/фильтры/карточка | извлечение: `metadata/fb2_provider.go::scanFb2EditionMeta` → `enricher.go::EnsureEditionMeta` (нормализация `normalizeLangCode`, грабля №14) · **внешний backfill** (перевод без fb2 `<src-lang>`): `metadata/src_lang_backfill.go` (клон year_backfill; источник ТОЛЬКО Wikidata P407 — `wikidata_srclang.go::FetchSrcLang`, precision-гейт «ровно один ISO-код» + гейт записи «≠ язык издания» в воркере: нативы src_lang НЕ получают; OL сознательно не источник — поля оригинала у него нет, languages = union изданий; учёт `book_src_lang_lookups` (миграция 0035), таргетный `UpsertWorksToIndex` изменённых работ; секция «Язык оригинала» в админке «Фоновые операции», API `api/admin_src_lang_enrichment.go`) · works-индекс `src_lang[]` (сырой) + производный **`orig_lang[]` — WORK-LEVEL эффективный оригинал** (схема **v8**): union непустых `src_lang` изданий работы, и ТОЛЬКО если src_lang нет ни у одного издания — union языков изданий (у изданий одной работы оригинал ОДИН, поэтому знание одного издания распространяется на соседей; перевод-сирота без fb2 `<src-lang>` — испанский «Sentido y Sensibilidad» при ru-соседе с src=en — больше НЕ считается испанским нативом, прод-кейс «Разум и чувства»/«Элегантность ёжика»): `importer/importer.go::workDocSelect` (трёхаргументный COALESCE двух подзапросов) + гейт `src_lang_synced_v1` (`main.go::runOnceSrcLangSync`) + авто-ресинк прогрева `prewarm.go::maybeResyncSrcLangs` · фильтры ПО `orig_lang` («оригинал: X» ловит и переводы с X, и нативы X; URL-параметр остаётся `?src_lang=`): `books/service.go::buildWorksFilter`, авторы `catalog/authors_list.go` (`src_langs` — work-level EXISTS с sibling-пробами по `books(work_id)`), опции `catalog/languages.go::ListSrcLanguages` (work-level: GROUP BY `COALESCE(work_id,-id)` + FILTER-агрегаты + unnest, счётчик = РАБОТЫ) · UI: `FiltersSidebar` (facetKey `orig_lang`) + `AuthorsPage::LanguagesFilter(src)` · титульная строка карточки: `lib/format.ts::translationLine` (по сырому src_lang — только у переводов) |
| Общие UI-примитивы | `frontend/src/components/ui/` (button, switch, callout, sheet, popover, dialog, …) |
| CSP фронта (ридер) и админка/OPDS «только из дома» | CSP — `frontend/nginx-security-headers.conf`, единственный источник: include в КАЖДЫЙ location `nginx.conf` (`add_header` в location отменяет заголовки уровня server) + тот же заголовок `vite preview` читает в `vite.config.ts` → Playwright гоняется под CSP. `script-src 'self'` без inline: foliate рендерит страницы книги в blob:-iframe'ах на нашем origin (sandbox с allow-same-origin+allow-scripts сам НЕ изолирует), они наследуют CSP → `<script>`/`onerror=` из fb2/EPUB не выполняются. ⚠️ Inline-скрипты запрещены везде — логика ридера в `public/foliate-reader.js`, не в HTML; внешние ресурсы (шрифты/картинки с чужих доменов) CSP тоже режет. e2e: фикстура `mockedPage` валит тест на любом нарушении CSP в консоли; `e2e/reader-csp.spec.ts` — «зловредный» EPUB (ZIP собирается прямо в тесте), без CSP все три вектора исполняются. Публичный деплой: `Caddyfile.public` пускает `/api/admin/*` (снаружи 403 JSON) и `/opds` (снаружи 404) только из `SKRIPTES_LAN_CIDRS` (`remote_ip` — адрес соединения, заголовками не подделать; не задана → 127.0.0.1/32 = закрыто для всех; overlay прокидывает переменную в caddy). Матчер пути Caddy нормализует регистр/%XX/`//`/`..` — проверено живым тестом |
