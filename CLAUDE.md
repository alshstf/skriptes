...
- `books.written_year` — год написания / первого издания произведения (fb2
  `<title-info><date>`; внешние источники — отдельным PR). Заполняется фоновым
  прогревом (`metadata/enricher.go::EnsureYearLocal` ← `fb2_provider.go::FetchYears`).
  По нему строится гистограмма «Книги по годам написания» на странице
  автора/серии (`catalog/service.go`); сюда же ляжет будущая bio-корреляция.
...
Текущая верхняя — `0012_book_year` (год написания/издания книги из fb2).
