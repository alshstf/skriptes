...
`SKRIPTES_TMDB_API_KEY` — без ключа TMDB 401, работает только P18; ключ
пробрасывают ОБА compose, атрибуция TMDB — в README). Провязка:
...
| Язык оригинала (src_lang) — фасет/фильтры/карточка | извлечение: `metadata/fb2_provider.go::scanFb2EditionMeta` → `enricher.go::EnsureEditionMeta` (нормализация `normalizeLangCode`, грабля №14) · **внешний backfill** (перевод без fb2 `<src-lang>`): `metadata/src_lang_backfill.go` (клон year_backfill; источник ТОЛЬКО Wikidata P407 — `wikidata_srclang.go::FetchSrcLang`, precision-гейт «ровно один ISO-код» + гейт записи «≠ язык издания» в воркере: нативы src_lang НЕ получают; OL сознательно не источник — поля оригинала у него нет, languages = union изданий; учёт `book_src_lang_lookups` (миграция 0035), таргетный `UpsertWorksToIndex` изменённых работ; секция «Язык оригинала» в админке «Фоновые операции», API `api/admin_src_lang_enrichment.go`) · works-индекс `src_lang[]` (сырой) + производный **`orig_lang[]` = src_lang ?? lang** (эффективный оригинал, схема v7): `importer/index.go` + гейт `src_lang_synced_v1` (`main.go::runOnceSrcLangSync`) + авто-ресинк прогрева `prewarm.go::maybeResyncSrcLangs` · фильтры ПО `orig_lang` («оригинал: X» ловит и переводы с X, и нативы X; URL-параметр остаётся `?src_lang=`): `books/service.go::buildWorksFilter`, авторы `catalog/authors_list.go` (`src_langs`, COALESCE-эффективный), опции `catalog/languages.go::ListSrcLanguages` (тоже эффективный) · UI: `FiltersSidebar` (facetKey `orig_lang`) + `AuthorsPage::LanguagesFilter(src)` · титульная строка карточки: `lib/format.ts::translationLine` (по сырому src_lang — только у переводов) |
