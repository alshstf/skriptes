...
Текущая верхняя — `0015_normalize_book_lang` (нормализация кодов языка к
lower+trim — см. граблю №14; до неё `0014_book_cover_lookups`). Backend хранит
...
трогают. Текущая версия — **0.6.3** (стабильный); `:latest` опубликован,
...
1. **Нормализация `lang`.** В INPX/fb2 один язык приходит в разном регистре
   (`ru` и `RU`), из-за чего он двоился в списке языков/фильтре/видимости (скрыть
   надо было каждый вариант отдельно). Импорт нормализует код:
   `importer.normalizeLang` (lower+trim) в `processRecord` → PG и Meili. Существующие
   данные чинит миграция `0015` (`UPDATE books SET lang = lower(btrim(lang))`), а
   Meili-индекс — разовый `Importer.ResyncLangs` на старте (гейт
   `app_settings.lang_normalized_v1`, см. `cmd/skriptes/main.go::runOnceLangResync`).
