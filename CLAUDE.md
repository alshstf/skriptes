...
трогают. Текущая версия — **0.7.1** (стабильный; модель «книга → издания»:
works, группировка, схлопывание поиска/каталога, секция «Издания»; патч 0.7.1 —
фикс ридера (SW navigateFallback), work-level favorite/read, обложка из любого
издания, coverage по живым изданиям); `:latest` опубликован, в `.env` можно
пинить `SKRIPTES_VERSION=latest`.
