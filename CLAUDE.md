...
| Postgres в интеграционных тестах | `backend/internal/testpg`: `Pool(t, ctx)` / `Start` (+DSN) / `DSN` (без миграций). Своих копий `postgres.Run` не заводить: общий хелпер ждёт и лог «ready» ×2, и опубликованный порт — без второго под параллельным `go test` ловился флак `port "5432/tcp" not found` |
