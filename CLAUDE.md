...
трогают. Текущая версия — **1.1.0** (★-избранное книг = служебная полка
`user_collections.kind='favorites'`, унифицировано с полками, миграция 0023 — см.
граблю №16; подписка на авторов/серии — иконка-колокольчик вместо звезды; личные
полки переехали в `/shelves` из меню пользователя). До неё **1.0.0** — первый
