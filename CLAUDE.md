...
read-only → молчаливый exit 1); **`"$var[...]"` в zsh — ИНДЕКСАЦИЯ МАССИВА**, а
не конкатенация (`bad math expression` → тихий exit 1; писать `${var}[` или не
копить вывод в переменную, а `echo`'ить сразу); `docker logs -f` умирает вместе
с контейнером при деплое/рестарте — прод-вотчер логов оборачивать в
reconnect-цикл или переармировать после каждого деплоя.

⚠️ **Вотчер-зомби (2026-07-19).** Monitor может остаться жив как процесс, но БЕЗ
output-файла в `<session>/tasks/<id>.output` — тогда его `echo` не доходят до
харнесса: событий нет, а `gh` дёргается каждую минуту. Снаружи выглядит как
«вотчер молчит», изнутри — «всё работает». Ловится сразу:
`ls <session>/tasks/<taskId>.output` — **нет файла = зомби**, `TaskStop` + проверить
`ps`. Замечено, когда TaskStop предыдущего вотчера и создание нового шли в
одном сообщении. Проверять существование output-файла после КАЖДОГО армирования
вотчера; и не держать вотчер, когда главное событие («CI зелёный») уже
отработано — мерж всё равно за пользователем.
...
идёт PR'ами: **PR-1 (сделан)** — данные + Wikidata-скелет; **PR-4 (сделан)** — блок
«В жизни автора в это время» на карточке книги (`authorevents/lifeevents.go::selectLifeEvents`
+ `GET /api/works/{id}/life-events` + `components/AuthorLifeSection.tsx`): окно
`[wy-3..wy]` для точечных, `during` для накрывающих периодов, плюс ОДНО «формирующее»
событие (weight 5, gap ≤40 лет) когда окно пусто — ловит отложенные связи вроде
Дрезден-1945 → «Бойня №5»-1969; **PR-3 (сделан)** —
таймлайн на карточке автора (`components/AuthorTimeline.tsx` + `lib/authorEvents.ts`;
⚠️ **Tailwind `row-span-*` компилируется в ШОРТКАТ `grid-row` и затирает
`grid-row-start` из `row-start-[…]`** — ячейки уезжают в авто-поток, ось
расходится с событиями; в таймлайне `grid-row` задан инлайном. Свёрнутый вид —
`collapseRows`, НЕ `slice`: у Толстого первые 12 строк — детство, книг в них нет
вовсе, а связь «книга ↔ событие» и есть смысл секции); **PR-2 (сделан)** —
Wikipedia-вехи (`metadata/wikipedia_events.go`: rule-based «предложение с годом» по
биозоне статьи; статья резолвится sitelink'ом QID — `WikidataEventsProvider.FetchSitelinks`,
НЕ opensearch по имени; дедуп wd↔wiki по (год,тип) с переносом цитаты в wd-строку,
общий кап 60 с якорями вне капа, атрибуция в ответе сервиса); PR-3/4 UI,
PR-5 воркер+админка+LLM. ⚠️ Go-regexp: `\w`/`\b` — ASCII-only, кириллицу НЕ матчат
(классификатор ловил это на «духовный кризис») — только явные классы/`\S`.
⚠️ **Правила экстрактора выстраданы живым прогоном на Достоевском/Толстом**
(2026-07-19), не ломать: биозона включает ПРЕАМБУЛУ (у Достоевского каторга
упомянута только там) и секцию «Семья» (смерть дочери Сони — строка списка
`Дочь София (1868—1868) … скончалась`, в Wikidata её нет вовсе; ловит
`familyDeathYear`); обрывки предложений отсеиваются по непарной скобке
(`danglingParen`) и началу не-с-заглавной; цитаты из писем/дневников — по
длинной или непарной `«…»` (`longQuote`), иначе реплика с «смертью» получает
вес утраты; названия в кавычках выкидываются ИЗ классификации («Война и мир»
в творческой вехе давала ложный `war`).
⚠️ pgx: nil-слайс в `= ANY($n)` кодируется NULL-массивом → `NOT(x=ANY(NULL))`=NULL —
prune молча не удаляет; пустой набор передавать `[]string{}`. Инварианты, которые нельзя ломать:
...
| Био-таймлайн автора (события жизни ⟷ книги) | Карточка книги: `authorevents/lifeevents.go` (`ListForWork`/`selectLifeEvents` — relation same_year∣during∣years_after∣delayed, окно+формирующее событие; ⚠️ связи ТОЛЬКО годовые) + `api/authorevents.go::handleWorkLifeEvents` + `components/AuthorLifeSection.tsx` (+`relationPhrase` в `lib/authorEvents.ts`) · UI автора: `components/AuthorTimeline.tsx` (двусторонний grid — события ∣ ось лет ∣ книги; на мобиле ось слева, книги под событиями с иконкой; подсветка окна `[Y-2..Y]` при наведении на книгу; схлопывание разрывов ≥4 лет; атрибуция CC0/CC BY-SA в подвале) + `lib/authorEvents.ts` (`useAuthorEvents` с поллингом до `done`, `buildTimeline`/`collapseRows`/`spansAt`/`relatedYears`) + секция `AuthorTimelineSection` в `AuthorPage` (не рендерится при `eligible=false`); e2e `frontend/e2e/author-timeline.spec.ts` · SPARQL-провайдер: `metadata/wikidata_events.go` (`WikidataEventsProvider` — один UNION-запрос P569/P570/P26/P40/P570-родных/P551/P69/P166/P793/P1344/P2632, `assembleWikidataEvents`: lifespan-фильтр, дедуп, кап наград ≤3, precision, русские формулировки; `ResolveAuthorQID` — фолбэк-резолв с occupation-гейтом) · пайплайн: `metadata/author_events.go::EnsureAuthorEvents` (renown-гейт + single-shot `events_fetched_at` + hidden-инвариант upsert'а) · QID-персист: `metadata/author_qid.go` + `WithQIDSink` на wikipedia/OL-провайдерах · читающий сервис + критерий eligible: `internal/authorevents/service.go` · API: `api/authorevents.go` (`GET /api/authors/{id}/events`, lazy-триггер) · грабля №21; план `~/.claude/plans/cryptic-roaming-turing.md` |
