...
// Обрывки предложений и строки семьи — оба кейса пойманы живым прогоном на
// Достоевском (см. PR-2/PR-4).
func TestSentenceFragmentsAndFamilyLines(t *testing.T) {
	t.Run("обрывок с непарной скобкой отбрасывается", func(t *testing.T) {
		// Реальный мусор из статьи: сплиттер разрезал внутри скобок, фрагмент
		// начинается с заглавной («Анны»), но это середина мысли.
		text := "== Биография ==\nАнны 2-й степени — 1832) и поселился в Даровом, где в 1839 году умер отец.\n"
		if got := extractWikiMilestones("ru", "u", text, 1821, 1881); len(got) != 0 {
			t.Fatalf("обрывок должен отсеиваться: %+v", got)
		}
	})

	t.Run("строка «Семьи» даёт смерть близкого по году из скобок", func(t *testing.T) {
		text := "== Семья ==\nДочь София (1868—1868) родилась в Женеве, где через несколько месяцев скончалась.\n"
		got := extractWikiMilestones("ru", "u", text, 1821, 1881)
		if len(got) != 1 || got[0].YearFrom != 1868 || got[0].Type != EventLoss {
			t.Fatalf("смерть дочери 1868: %+v", got)
		}
	})

	t.Run("строка о живом ребёнке смертью не считается", func(t *testing.T) {
		text := "== Семья ==\nСын Фёдор (1871—1922) родился в Петербурге.\n"
		for _, ev := range extractWikiMilestones("ru", "u", text, 1821, 1881) {
			if ev.Type == EventLoss {
				t.Fatalf("год смерти из скобок без маркера смерти брать нельзя: %+v", ev)
			}
		}
	})

	t.Run("преамбула входит в биозону", func(t *testing.T) {
		text := "Был осуждён по делу петрашевцев в 1849 году на четыре года каторги.\n== Библиография ==\nМ.: Наука, 1972.\n"
		got := extractWikiMilestones("ru", "u", text, 1821, 1881)
		if len(got) != 1 || got[0].Type != EventPersecution {
			t.Fatalf("факт из преамбулы должен извлекаться: %+v", got)
		}
	})
}

// Цитаты из писем/дневников — не события (живой прогон: реплика графини
// Толстой получала вес утраты из-за слова «смертью»).
func TestQuotesAreNotEvents(t *testing.T) {
	cases := []string{
		`В частности, графиня писала: «Заявляю ещё, что Лев Николаевич ни разу перед смертью не вспомнил обо мне» в 1899 году.`,
		`В 1899 году он записал: «Жизнь моя вся переменилась и я не тот, что был`, // непарная кавычка
	}
	for _, sent := range cases {
		text := "== Биография ==\n" + sent + "\n"
		if got := extractWikiMilestones("ru", "u", text, 1828, 1910); len(got) != 0 {
			t.Fatalf("цитата не должна становиться вехой (%q): %+v", sent[:40], got)
		}
	}
	// Название произведения в кавычках короткое — веху не убивает.
	text := "== Биография ==\nВ 1869 году завершил роман «Война и мир» и отдал его в печать.\n"
	if got := extractWikiMilestones("ru", "u", text, 1828, 1910); len(got) != 1 {
		t.Fatalf("короткие кавычки не должны отсеивать веху: %+v", got)
	}
}
