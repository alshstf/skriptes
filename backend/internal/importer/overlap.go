...
// Если раздача переименовала INPX (lib.rus.ec, сентябрь 2026:
// librusec_local_fb2.inpx → librusec_flib.inpx + librusec_mhl.inpx) или рядом
// лежат два INPX одной библиотеки, импорт завёл бы новую коллекцию и вставил
// каждую книгу второй раз (#250). Такой файл не импортируется. Что именно
// отличает варианты одной раздачи (flib/mhl), не выясняли — поэтому
// «переименованный» INPX не подхватывается как старая коллекция автоматически:
// решает владелец, оставив в каталоге один файл под прежним именем.
